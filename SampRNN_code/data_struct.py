...
def canonicalize_smiles_from_file(fname):
    ...
    with open(fname, 'r') as f:
        ...
        for i, line in enumerate(f):
            ...
            #if filter_mol(mol):         #看用哪个文件，如果用filter的smi，就可以加这个，不然就别加

            smiles_list.append(Chem.MolToSmiles(mol,isomericSmiles=False))
...
def write_smiles_to_file(smile_list,fname):
...
#     smiles_file = sys.argv[1]
    ...
    smiles_file = './COCONUT_DB_train_list.smi'
    ...
    voc_chars = construct_vocabulary(smiles_list, "./Voc")

    write_smiles_to_file(smiles_list, "./mols_filtered.smi")
